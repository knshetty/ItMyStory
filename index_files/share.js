    ...
    _tileload: function(e) {
        ...
        if (tileUrl in this.tileStats){
            ...
            this.events.push({
                ...
                url: tileUrl,
                timeTaken: this.tileStats[tileUrl].tileloadtime
            });
        }
    },
...
},{"mapbox-events":2}],2:[function(require,module,exports){
var xhr = require('xhr');
...
},{"hat":3,"lodash-compat":4,"xhr":5}],3:[function(require,module,exports){
...
},{"./marker":33,"./request":34,"./simplestyle":36,"./url":38,"./util":39,"sanitize-caja":42}],19:[function(require,module,exports){
'use strict';
...
},{"./feedback":19,"./request":34,"./url":38,"./util":39}],21:[function(require,module,exports){
'use strict';
...
},{"./util":39,"mustache":15,"sanitize-caja":42}],24:[function(require,module,exports){
'use strict';
...
},{"sanitize-caja":42}],27:[function(require,module,exports){
window.L = require('leaflet/dist/leaflet-src');

},{"leaflet/dist/leaflet-src":14}],28:[function(require,module,exports){
'use strict';
...
},{"sanitize-caja":42}],29:[function(require,module,exports){
'use strict';
...
},{"./request":34,"./url":38,"./util":39}],30:[function(require,module,exports){
'use strict';
...
},{"./feature_layer":18,"./feedback":19,"./grid_control":23,"./grid_layer":24,"./info_control":26,"./legend_control":28,"./load_tilejson":29,"./mapbox_logo":32,"./share_control":35,"./tile_layer":37,"./util":39,"sanitize-caja":42}],31:[function(require,module,exports){
'use strict';
...
},{"../package.json":16,"./config":17,"./feature_layer":18,"./feedback":19,"./geocoder":20,"./geocoder_control":21,"./grid_control":23,"./grid_layer":24,"./info_control":26,"./legend_control":28,"./map":30,"./marker":33,"./share_control":35,"./simplestyle":36,"./tile_layer":37,"mustache":15,"sanitize-caja":42}],32:[function(require,module,exports){
'use strict';
...
},{"./url":38,"./util":39,"sanitize-caja":42}],34:[function(require,module,exports){
'use strict';
...
},{"./config":17,"./util":39,"corslite":13}],35:[function(require,module,exports){
'use strict';
...
},{"./load_tilejson":29,"./url":38}],36:[function(require,module,exports){
'use strict';
...
},{"./load_tilejson":29,"./util":39,"sanitize-caja":42}],38:[function(require,module,exports){
'use strict';
...
},{"../package.json":16,"./config":17}],39:[function(require,module,exports){
'use strict';
...
window.onload = function() {
    ...
    map.featureLayer.on('ready', function(e) {
        ...
        if (tilejson.description) {
            var descTray = document.getElementById('desc-tray');
            descTray.innerHTML = "<p class='pad1'>" + stripHTML(tilejson.description) + "</p>";

            var wrapper = document.getElementById('overlay-wrapper');
            L.DomUtil.addClass(wrapper,'hasdesc');
        }

        function layerClick(e) {
            var item = document.getElementById(e.target.feature.properties.id),
                parentNode = item.parentNode,
                parentOffset = parentNode.offsetTop + parentNode.clientTop,
                offset = item.offsetTop - parentOffset;

            if (!L.DomUtil.hasClass(item, 'active')) item.parentNode.scrollTop = offset;
        }
        ...
        if (layerLength) {
            for (var i = 0; i < layerLength; i++) {
                layers[i].on('click', layerClick)
                    .on('popupopen', layerPopupOpen)
                    .on('popupclose', layerPopupClose);
            }

            var wrapper = document.getElementById('overlay-wrapper');
            L.DomUtil.addClass(wrapper,'hasmarkers');

            var markerLink = "<a id='marker-toggle' href='#' class='hide-mobile round-right small block dark center pin-right pad1 icon marker strong'>" + layerLength + "</a>";
            document.getElementById('overlay-compressed').innerHTML += markerLink;

            var markerTray = document.getElementById('marker-tray');
            markerTray.innerHTML = templateMarkers(e.target.getGeoJSON().features);

            var markers = markerTray.getElementsByTagName('a'),
                markerLength = markers.length;

            for (var k = 0; k < markerLength; k++) {

                markers[k].addEventListener('click', function(e) {
                    L.DomEvent.stop(e);
                    var id = e.currentTarget.id,
                        layers = map.featureLayer.getLayers(),
                        marker;

                    for (var l = 0; l < layers.length; l++) {
                        if (id == layers[l].feature.properties.id) {
                            marker = layers[l];
                        }
                    }

                    var type = marker.feature.geometry.type;

                    for (var m = 0; m < markerLength; m++) {
                        if (markers[m].id === id) {
                            L.DomUtil.addClass(markers[m], 'active');
                        } else {
                            L.DomUtil.removeClass(markers[m], 'active');
                        }
                    }

                    var compressed = document.getElementById('overlay-compressed'),
                        offset = (compressed.offsetHeight + document.getElementById('marker-tray').offsetHeight) / 2,
                        crs = map.options.crs,
                        zoom = map.getZoom();

                    if (type === 'Point') {
                        var markerPoint = crs.latLngToPoint(marker.getLatLng(), zoom);
                        markerPoint.y -= offset;
                        if (window.outerWidth > 640) { markerPoint.x -= offset };
                        map.panTo(crs.pointToLatLng(markerPoint, zoom));
                        marker.openPopup();
                    } else {
                    ...
                    if (window.outerWidth < 640) {
                        ...
                        L.DomUtil.addClass(shareOverlay, 'compressed');
                    }
            ...
            L.DomEvent.addListener(L.DomUtil.get('marker-toggle'), 'click', function(e) {
                L.DomEvent.stop(e);
                var shareOverlay = document.getElementById('share-overlay');
                toggleClass(shareOverlay, 'compressed');
            });

            if (window.outerWidth < 640) {
                L.DomEvent.addListener(L.DomUtil.get('overlay-wrapper'), 'click', function(e) {
                    L.DomEvent.stop(e);
                    var shareOverlay = document.getElementById('share-overlay');
                    toggleClass(shareOverlay, 'compressed');
                });
    ...
    function toggleClass(el, className) {
        return L.DomUtil.hasClass(el, className) ?
            L.DomUtil.removeClass(el, className) : L.DomUtil.addClass(el, className);
    }
    ...
    function templateMarkers(markers) {
        ...
        for (var i = 0; i < length; i++) {
            var type = markers[i].geometry.type.toLowerCase(),
                title = stripHTML(markers[i].properties.title);
            ...
            markerList += '<a class="truncate keyline-bottom block contain pad1 small" id="' + markers[i].properties.id + '">' +
                            '<span class="quiet icon ' + icon + '">';
            if (title.length > 0) {
                markerList += title;
            } else markerList += '<em>' + desc + '</em>';
...
},{"instrumentile":1,"leaflet-hash":12,"mapbox.js":25,"underscore":40}],42:[function(require,module,exports){
